"""Интеграционные тесты для POST /api/v1/offers/run. ML не вызывается — мокаем get_calculate_items."""
...
async def test_offers_run_success(aiohttp_client, app):
    """POST /offers/run возвращает 200 и список items при успешном расчёте (ML замокан)."""
    mock_result = {
        "items": [
...
async def test_offers_run_ml_error_returns_500(aiohttp_client, app):
    """При ошибке ML (get_calculate_items raises) возвращается 500."""
    with patch("functions.calculate.get_calculate_items", new_callable=AsyncMock) as mock_get:
        mock_get.side_effect = Exception("ML service unreachable")

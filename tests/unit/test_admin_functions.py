...
class TestAdminAuthFunctions:
    ...
    async def test_create_admin_success(self):
        ...
        with patch.object(auth_fns, 'get_admin_by_login', new=AsyncMock(return_value={"admin_id": 5, "login": "newadmin", "approved": False})):
            with patch.object(auth_fns, 'Database') as MockDB:
                ...
                result = await auth_fns.create_admin(
                    test_admin["login"], test_admin["password"]
                )
    ...
    async def test_set_admin_approved_success(self):
        with patch.object(auth_fns, 'get_admin_by_id', new=AsyncMock(return_value={
            "admin_id": 2,
            "login": "admin2",
            "main_admin": False,
            "approved": True,

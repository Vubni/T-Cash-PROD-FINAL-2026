...
def upgrade() -> None:
    ...
    # 2) categories.rule_id: снять FK, перевести в UUID, вернуть FK
    ...
    # 3) categories.category_id и selections.category_id, selection_id
    op.drop_constraint(
    ...
    # 4) selections.selection_id: TEXT -> UUID (новые UUID для существующих значений)
    op.add_column(
        "selections",
...
def downgrade() -> None:
    # selection_id: UUID -> TEXT (без восстановления старых значений)
    op.drop_constraint("selections_selection_id_key", "selections", type_="unique")
    op.add_column(
        "selections",

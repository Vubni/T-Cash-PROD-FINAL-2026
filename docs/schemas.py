...
class LoginEditSchema(Schema):
    login = fields.Str(
        required=True,
        description="Новый логин пользователя. Уникальный, до 20 символов. По нему пользователь будет входить.",
    )
...
class EmailEditSchema(Schema):
    email = fields.Str(
        required=True,
        description="Новый email пользователя. До 256 символов. После смены может потребоваться верификация.",
    )
...
class ClubNewSchema(Schema):
    ...
    max_members_counts = fields.Int(
        required=False, missing=0, description="Максимальное количество участников клуба (по умолчанию 0)"
    )
    class_limit_min = fields.Int(
        required=False, missing=1, description="Минимальный класс для участия в клубе (по умолчанию 1)"
    )
    class_limit_max = fields.Int(
        required=False, missing=11, description="Максимальный класс для участия в клубе (по умолчанию 11)"
    )
    telegram_url = fields.Str(
        required=False, allow_none=True, missing=None, description="URL телеграм-канала клуба (опционально)"
    )
...
class ScheduleItemSchema(Schema):
    ...
    title = fields.Str(
        required=False, allow_none=True, description="Название урока (присутствует только если урок запланирован)"
    )
    classrooms = fields.List(
        fields.Raw(), required=False, allow_none=True, description="Список кабинетов (строки/числа) или None"
    )
    teachers = fields.List(
        fields.Str(), required=False, allow_none=True, description="Список преподавателей (строки) или None"
    )
...
class AchievementsGlobalReturnSchema(Schema):
    ...
    xp = fields.Str(
        description="Текущий набранный опыт пользователя по этому достижению (строка для гибкости формата)."
    )
    need_xp = fields.Str(
        description="Порог опыта, необходимый для получения достижения (строка). Сравнивать с xp для прогресс-бара."
    )
...
class AchievementsSchema(Schema):
    ...
    image_path = fields.Str(
        description="Относительный путь к картинке достижения; склеивать с base URL медиа при отображении."
    )
...
class TelegramAuthSchema(Schema):
    url = fields.Str(
        description="Ссылка на Telegram-бота для привязки аккаунта; перенаправлять пользователя по этой ссылке."
    )
    token = fields.Str(
        description="Токен для опроса статуса: прошла ли авторизация в боте (использовать в последующих запросах проверки)."
    )
...
class ForgotPasswordSchema(Schema):
    ...
    new_password = fields.Str(
        required=True,
        description="Новый пароль, который пользователь хочет установить после перехода по ссылке из письма.",
    )
...
class FieldErrorItemSchema(Schema):
    ...
    issue = fields.Str(
        description="Описание проблемы (например, «must be non-negative», «invalid UUID»). Показывать под полем."
    )
    rejectedValue = fields.Raw(
        allow_none=True,
        description="Значение, которое отклонил сервер; может быть null. Полезно для отладки и подсказок.",
    )


class HttpErrorSchema(Schema):
    code = fields.Str(
        description="Строковый код ошибки: UNAUTHORIZED (401), FORBIDDEN (403), NOT_FOUND (404), VALIDATION_ERROR (422) и т.д. Использовать для ветвления логики на фронте."
    )
    message = fields.Str(description="Краткое сообщение об ошибке для пользователя или логов.")
    traceId = fields.Str(
        description="Уникальный идентификатор запроса; передавать в поддержку при обращении по ошибке."
    )
    ...
    details = fields.Dict(
        allow_none=True, description="Дополнительные данные (объект); зависит от эндпоинта, может быть null."
    )
...
class AlreadyBeenTaken(Schema):
    name = fields.Str(
        description="Имя поля (например login или email), значение которого уже занято другим пользователем."
    )
...
class CategoryRateSchema(Schema):
    min = fields.Float(
        required=True,
        description="Минимальная ставка кэшбэка в процентах (0–100). Показывается пользователю как нижняя граница диапазона. Обязательное поле.",
    )
    max = fields.Float(
        required=True,
        description="Максимальная ставка кэшбэка в процентах (0–100). Показывается пользователю как верхняя граница диапазона. Обязательное поле.",
    )
...
class CategoryRuleSchema(Schema):
    rule_id = fields.Str(
        required=True,
        description="UUID правила отбора. Связь категории с правилом (возраст, пол, доход). Обязательное поле.",
    )
    min_age = fields.Int(
        allow_none=True,
...
class CategoryHistoryItemSchema(Schema):
    ...
    changed_by = fields.Str(
        required=True, description="Идентификатор или логин того, кто внёс изменение (для аудита). Обязательное поле."
    )
    field = fields.Str(
        required=True, description="Имя поля, которое изменили (name, budget_amount и т.д.). Обязательное поле."
    )
    old_value = fields.Raw(
        required=False,
        allow_none=True,
        description="Значение до изменения; может быть null для сложных типов. Опционально.",
    )
    new_value = fields.Raw(
        required=False,
        allow_none=True,
        description="Значение после изменения; может быть null для сложных типов. Опционально.",
    )


class CategoryListItemSchema(Schema):
    id = fields.Str(
        required=True, description="UUID категории. Используется в path и в запросах выбора. Обязательное поле."
    )
    name = fields.Str(
        required=True, description="Название категории для отображения (до 500 символов). Обязательное поле."
    )
    subtitle = fields.Str(
        required=True,
    ...
    rate = fields.Nested(
        CategoryRateSchema,
        required=True,
        description="Диапазон ставок кэшбэка: min и max в процентах для отображения пользователю.",
    )
    status = fields.Str(
        required=True,
...
class CategoryListResponseSchema(Schema):
    ...
    total = fields.Int(
        required=True,
        description="Общее число категорий в системе; нужно для пагинации (например, «показано 10 из total»). Обязательное поле.",
    )
...
class CategoryCreateSchema(Schema):
    name = fields.Str(
        ...
        description="Название категории для отображения пользователю (1–500 символов). Обязательное поле.",
    )
    subtitle = fields.Str(
        required=True,
        validate=validate.Length(min=1, max=NAME_SUBTITLE_MAX),
...
class CategoryUpdateSchema(Schema):
    name = fields.Str(
        required=False,
        validate=validate.Length(min=1, max=NAME_SUBTITLE_MAX),
        description="Новое название категории (1–500 символов). Опционально.",
    )
    subtitle = fields.Str(
        required=False,
        validate=validate.Length(min=1, max=NAME_SUBTITLE_MAX),
...
class AdminRegisterSchema(Schema):
    login = fields.Str(
        required=True,
        validate=validate.Length(min=4, max=ADMIN_LOGIN_MAX),
        description="Логин нового администратора (4–20 символов). Уникальный. Обязательное поле.",
    )
    password = fields.Str(
        required=True,
        validate=validate.Length(min=4, max=ADMIN_PASSWORD_MAX),
        description="Пароль администратора (4–20 символов). После регистрации нужно одобрение главным админом. Обязательное поле.",
    )
...
class AdminLoginSchema(Schema):
    login = fields.Str(
        required=True,
        validate=validate.Length(min=4, max=ADMIN_LOGIN_MAX),
        description="Логин администратора (4–20 символов). Обязательное поле.",
    )
    password = fields.Str(
        required=True,
        validate=validate.Length(min=4, max=ADMIN_PASSWORD_MAX),
        description="Пароль администратора (4–20 символов). Обязательное поле.",
    )
...
class AdminAuthResponseSchema(Schema):
    ...
    token = fields.Str(
        required=True,
        description="JWT для запросов в админ-API. Передавать в заголовке Authorization: Bearer <token>. Обязательное поле.",
    )
...
class AuditEventSchema(Schema):
    ...
    entity_type = fields.Str(
        required=True,
        validate=validate.Length(min=1, max=AUDIT_ENTITY_TYPE_MAX),
        description="Тип сущности (1–50 символов): category, rule, admin, client. Обязательное поле.",
    )
    entity_id = fields.Str(
        required=True,
        validate=validate.Length(min=1, max=AUDIT_ENTITY_ID_MAX),
        description="ID сущности (1–64 символа). Обязательное поле.",
    )
    action = fields.Str(
        required=True,
        validate=validate.Length(min=1, max=AUDIT_ACTION_MAX),
        description="Тип действия (1–50 символов): create, update, delete, selection. Обязательное поле.",
    )
    actor = fields.Str(
        required=True,
...
class UserSchema(Schema):
    id = fields.Int(
        required=True, description="Внутренний ID пользователя (BIGINT). Для отображения в админке. Обязательное поле."
    )
...
class CalculateCategoryItemSchema(Schema):
    ...
    category_id = fields.Str(
        required=True,
        description="UUID категории. Используется при отправке выбора (category_ids) и в эндпоинтах выбора. Обязательное поле.",
    )
...
class SelectionDetailSchema(Schema):
    selection_id = fields.Str(
        required=True,
        description="UUID сохранённого выбора. Используется в эндпоинтах подтверждения и в расчёте. Обязательное поле.",
    )
...
class SelectionConfirmResponseSchema(Schema):
    ...
    category_id = fields.Str(
        required=True, description="UUID категории, по которой подтверждён выбор. Обязательное поле."
    )
    expected_benefit_amount = fields.Float(
        required=True,
    ...
    message = fields.Str(
        required=True,
        description="Сообщение от бэкенда (успех или предупреждение). Показать пользователю. Обязательное поле.",
    )
...
class ProgressItemSchema(Schema):
    ...
    status = fields.Str(
        description="Статус обработки/прогресса по этой записи (например, pending, confirmed). Используйте для отображения прогресса. Опционально."
    )
